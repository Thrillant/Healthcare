...
export const getAppointments = async (req, res) => {
    try {
        ...
        const items = await Appointment.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).populate("doctorId", "name specialization owner imageUrl image");
...
export const createAppointment = async (req, res) => {
    try {
        ...
        if (existingBooking) {
            return res.status(400).json({
                success: false,
                message: "You already have an appointment booked at this date and time with this doctor."
            });
        }
...
// Confirm the online payment and make it paid
export const confirmPayment = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Payment confirmed successfully", appointment: appt });
    }
    catch (err) {
...
export const updateAppointment = async (req, res) => {
    try {
        ...
        if (!updated) {
            return res.status(404).json({ success: false, message: "Failed to update appointment" });
        }
...
export const cancelAppointment = async (req, res) => {
    try {
        ...
        await appt.save();
        return res.status(200).json({ success: true, message: "Appointment canceled successfully", appointment: appt });
    }
    catch (err) {
...
export const getAppointmentByDoctor = async (req, res) => {
    try {
        ...
        console.error("GetAppointmentByDoctor unexpected:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
