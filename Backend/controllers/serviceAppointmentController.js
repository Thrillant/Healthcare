...
export const createServiceAppointment = async (req, res) => {
    try {
        ...
        console.error("Create Service Appointment Error:", err);
...
// Get Service Apppointment by Id
export const getServiceAppointmentById = async (req, res) => {
    try {
        const { id } = req.params;
        const appt = await ServiceAppointment.findById(id).populate("serviceId", "name image imageUrl imageSmall").lean();
        if (!appt) {
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        // Check whether it is filled if yes then update
...
export const cancelServiceAppointment = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            data: appt,
            message: "Service appointment cancelled successfully"
        });
    }
    catch (err) {
...
export const getServiceAppointmentStats = async (req, res) => {
    try {
        ...
        // Gives total Appointments, completed, canceled and total earning by them

        return res.status(200).json({
            success: true,
            services,
            totalServices: services.length,
            message: "Service appointments stats fetched successfully"
        });
    }
    catch (err) {
        console.error("Get Service Appointment Statistics Error:", err);
        return res.status(500).json({
            success: false,
            message: "Server Error"
        });
