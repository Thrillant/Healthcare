...
import { appointmentPageStyles as aps, cardStyles, badgeStyles, iconSize } from "../assets/dummyStyles";
import axios from "axios";
import { CreditCard, Wallet, Bell, CheckCircle, Clock, XCircle, CalendarDays, Calendar, User } from "lucide-react";
...
// Helper Functions
// Formats a number with a leading zero if needed
function pad(n) {
...
// Computes the status of an appointment based on the current date and time
...
// PaymentBadge component to display payment status
...
// StatusBadge component to display appointment status
...
const AppointmentPage = () => {
    ...
    // Normalize Rescheduled Appointments
    ...
    return (
        <div className={aps.pageContainer}>
            ...
            <div className={aps.maxWidthContainer}>
                <h1 className={aps.doctorTitle}>
                    Your Doctor Appointments
                </h1>
                {loadingDoctors && (
                    <div className={aps.loadingText}>Loading Doctors...</div>
                )}

                {!loadingDoctors && appointmentData.length === 0 && (
                    <div className={aps.emptyStateText}>No appointments found!</div>
                )}

                <div className={aps.doctorGrid}>
                    {appointmentData.map((apt) => (
                        <div key={apt.id} className={cardStyles.doctorCard}>
                            <div className={cardStyles.doctorImageContainer}>
                                <img src={apt.image || "/placeholder-doctor.png"} alt={apt.doctor} className={cardStyles.image} loading="lazy" />

                                <h2 className={cardStyles.doctorName}>{apt.doctor}</h2>
                                <div className={cardStyles.specialization}>
                                    {apt.specialization}{" "}
                                    {apt.experience ? `| ${apt.experience} Years` : ""}
                                </div>

                                <p className={cardStyles.dateContainer}>
                                    <CalendarDays className={iconSize.medium} /> {apt.date}
                                </p>
                                <p className={cardStyles.timeContainer}>
                                    <Clock className={iconSize.medium} /> {apt.time}
                                </p>
                                <p className={cardStyles.badgesContainer}>
                            ...
                            {apt.status === "Rescheduled" && apt.rescheduledTo ? (
                                <div className={cardStyles.rescheduledText}>
                                    ...
                                    <span className={cardStyles.rescheduledSpan}>
                                        {apt.rescheduledTo.date} : {apt.rescheduledTo.time}
                                    </span>
                                </div>
                            ) : null}
                        </div>
                ...
                <h1 className={aps.serviceTitle}>
                    Your Service Appointments
                </h1>
                {loadingServices && (
                    <div className={aps.serviceLoadingText}>Loading Service Bookings...</div>
                )}

                {!loadingServices && serviceData.length === 0 && (
                    <div className={aps.serviceEmptyStateText}>No service bookings found!</div>
                )}
