...
function StatusBadge({ status }) {
    const base = lps.statusBadgeBase;
    if (status === "complete")
        return (
            <span className={`${base} ${lps.statusBadgeComplete}`}>
                Completed
            </span>
        );
    if (status === "cancelled")
        return (
            <span className={`${base} ${lps.statusBadgeCancelled}`}>
                Cancelled
            </span>
        );
    if (status === "confirmed")
        return (
            <span className={`${base} ${lps.statusBadgeConfirmed}`}>
                Confirmed
            </span>
        );
    if (status === "rescheduled")
        return (
            <span className={`${base} ${lps.statusBadgeRescheduled}`}>
                Rescheduled
            </span>
        );
    return (
        <span className={`${base} ${lps.statusBadgePending}`}>
            Pending
        </span>
    );
}
    ...
    if (appointment.status === "rescheduled") {
        return (
            <select
                ...
                className={`${lps.statusSelect} ${terminal
                    ? lps.statusSelectDisabled
                    : lps.statusSelectEnabled
                    }`}
            ...
            {!editing ? (
                <div className="flex justify-end">
                    <button
                        ...
                        className={`${lps.rescheduleButton} ${terminal
                            ? lps.rescheduleButtonDisabled
                            : lps.rescheduleButtonEnabled
                            }`}
                    >
                        Reschedule
                    </button>
                ...
                <div className={lps.rescheduleForm}>
                    <input
                        type="date"
                        value={date}
                        min={minDate}
                        onChange={(e) => setDate(e.target.value)}
                        className={lps.dateInput}
                    />
                    <input
                        type="time"
                        value={time}
                        onChange={(e) => setTime(e.target.value)}
                        className={lps.timeInput}
                    />
                    <div className={lps.rescheduleButtons}>
                        <button onClick={save} className={lps.saveButton}>
                            Save
                        </button>
                        <button onClick={cancel} className={lps.cancelButton}>
                            Cancel
                        </button>
                    </div>
                </div>
            ...
            <div className={lps.contentWrapper}>
                <div className={lps.headerContainer}>
                    <div>
                        ...
                        <p className={lps.headerSubtitle}>Latest at top - search by patient name</p>
                    </div>
                    ...
                    <div className={lps.searchFilterContainer}>
                        <div className={lps.searchContainer}>
                            ...
                            {search && (
                                ...
                                >
                                    <X className={lps.clearSearchIcon} />
                                </button>
                            )}
                        </div>
                ...
                {
                    loading ? (
                        <div className={lps.loading}>Loading appointments...</div>
                    ) : error ? (
                        <div className={lps.error}>{error}</div>
                    ) : (
                                ...
                                <article key={a.id} className={lps.appointmentCard}>
                                    <header className={lps.cardHeader}>
                                        ...
                                        <div className={lps.cardContent}>
                                            <div className={lps.cardPatientName}>
                                                {a.patient}
                                            </div>
                                            ...
                                            <div className={lps.cardDoctorInfo}>
                                                <span className={lps.cardDoctorName}>{a.doctorName}</span>
                                            </div>
