...
import { serviceDashboardStyles as sds } from "../assets/dummyStyles";
...
const ServiceDashboardPage = ({ services: servicesProp = null }) => {
    ...
    // Confirms that services are present
    useEffect(() => {
        ...
        // Refresh the page when the tab is focused
        function onFocus() {
            fetchServices({ showLoading: false });
        }
        ...
        // Refresh the page when the tab is visible
    ...
    // Stats of total appointments, completed, canceled and earnings
            ...
            <div className={sds.innerContainer}>
                ...
                <div className={sds.header.container}>
                    ...
                    <div className={sds.refresh.container}>
                        ...
                        <button onClick={() => {
                            ...
                            title={Array.isArray(servicesProp) ? "Services" : "Refresh"}
                        >
                            Refresh
                        </button>
                    </div>
                ...
                <div className={sds.table.container}>
                    ...
                    <div className={sds.table.body}>
                        {loading ? (
                            <div className={sds.states.loading}>
                                Loading Services...
                            </div>
                        ) : error ? (
                            <div className={sds.states.error}>
                                Error: {error}
                            </div>
                        ) : visibleServices.length === 0 ? (
                            <div className={sds.states.empty}>
                                No services found
                            </div>
                        ) : (
                            visibleServices.map((s) => {
                                ...
                                return (
                                    <div key={s.id} className={sds.table.row}>
                                        {/* For tablet view */}
                                        <div className={sds.table.tabletView}>
                                            <div className="flex items-center gap-3">
                                                <div
                                                    className={sds.table.tabletImage}
                                                >
                                                    <img
                                                        src={s.image}
                                                        alt={s.name}
                                                ...
                                                <div
                                                    className={
                                                        sds.table.tabletTextContainer
                                                    }
                                                >
                                                    <div
                                                        className={
                                                            sds.table.tabletServiceName
                                                        }
                                                    >
                                                        {s.name}
                                                    </div>
                                                    <div
                                                        className={sds.table.tabletPrice}
                                                    >
                                            ...
                                            <div
                                                className={`${sds.table.tabletCell} text-emerald-700`}
                                            >
                                                {s.completed}
                                            </div>
                                            <div
                                                className={`${sds.table.tabletCell} text-red-500`}
                                            >
                                                {s.canceled}
                                            </div>
                                            <div
                                                className={`${sds.table.tabletCell} text-right`}
                                            >
                                        ...
                                        <div className={sds.table.desktopView}>
                                            ...
                                            <div className={sds.table.desktopCell(2)}>{formatCurrency(s.price)}</div>
                                            <div className={sds.table.desktopCenterCell(1)}>{s.totalAppointments}</div>
                                            <div className={sds.table.desktopCenterCell(1)}>{s.completed}</div>
                                            <div className={sds.table.desktopCenterCell(1)}>{s.canceled}</div>
                                            <div className={`${sds.table.desktopCell(2)} text-right`}>{formatCurrency(earning)}</div>
                                        </div>
                                        ...
                                        <div className={sds.table.mobileView}>
                                            <div className="flex items-start gap-3">
                                                <div
                                                    className={sds.table.mobileImage}
                                                >
                                                    <img
                                                        src={s.image}
                                                        alt={s.name}
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div
                                                        className={
                                                            sds.table.mobileServiceHeader
                                                        }
                                                    >
                                                        <h3
                                                            className={
                                                                sds.table.mobileServiceName
                                                            }
                                                        >
                                                            {s.name}
                                                        </h3>
                                                    ...
                                                    <div
                                                        className={
                                                            sds.table.mobileStatsContainer
                                                        }
                                                    >
                                                        <div
                                                            className={sds.table.mobileStatItem(
                                                                "emerald",
                                                            )}
                                                        >
                                                            <Calendar size={14} />
...
function StatCard({ icon, label, value }) {
    return (
        <div className={sds.statCard.container}>
            <div className={sds.statCard.iconContainer}>
                {icon}
            </div>
            <div>
