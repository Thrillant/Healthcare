...
import { serviceAppointmentsStyles as sas } from "../assets/dummyStyles";
import { SearchIcon, XIcon, CheckCircle, XCircle, Phone, Calendar, Clock, User, BadgeIndianRupee, Loader2 } from "lucide-react";
...
function RescheduleButton({ appointment, onReschedule, disabled }) {
    ...
    // Saves the rescheduled appointment
    function save() {
        if (!date || !time) return;
            ...
            {!editing ? (
                ...
                <div className={sas.rescheduleEditContainer}>
                    ...
                    <div className={sas.rescheduleActions}>
                        <button
                            onClick={save}
                            className={sas.rescheduleSaveButton}
                        >
                            Save
                        </button>
                        <button
                            onClick={cancel}
                            className={sas.rescheduleCancelButton}
                        >
                            Cancel
                        </button>
                    </div>
...
const ServiceAppointmentsPage = () => {
    ...
    // Get the timestamp of the appointment
            ...
            <header className={sas.headerContainer}>
                ...
                <div className={sas.searchContainer}>
                        ...
                        <label className={sas.searchLabel}>
                            ...
                            <div className="flex items-center gap-2 relative w-full">
                                <div className={sas.searchIconContainer}>
                                    <SearchIcon className={sas.searchIcon} />
                                </div>
                                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by patient or service..." className={sas.searchInput} />
                                {search ? (
                                    <button className={sas.clearSearchButton} onClick={() => setSearch("")}>
                    ...
                    <div className={sas.searchInfo}>
                        ...
                        <div>
                            <button onClick={fetchAppointments} className={sas.refreshButton}>
                                Refresh
                            </button>
                        </div>
                    </div>
                ...
                <div className={sas.gridContainer}>
                    {displayList.length === 0 ? (
                        ...
                        displayList.map((appt) => {
                            ...
                            return (
                                <article key={appt.id} className={sas.article}>
